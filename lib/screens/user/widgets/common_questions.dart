...
"""),
  ...
  void setSearchResultList() {
    ...
    if(query != "") {
      questionCards.forEach((element) {
        String question = element.question.toLowerCase();
        if(question.contains(query)){
          showResults.add(element.getQuestionCard());
        }
      });
    }
    else {
      showResults = List.from(questionCards.map((e) => e.getQuestionCard()));
    }

    setState(() {
      resultCards = showResults;
    });
  }
